...
const NavbarAuth = ({ LinkComponent, logo, logoHeight, logoWidth, avatarUrl, userName, nickname, bgColor = 'white', callToAction, callToActionName, links, Collapse }: IProps) => {
                ...
                <div className="flex gap-16 items-center">
                    ...
                    <div className="flex gap-14">
                        {links.map(({ name, link, active }, index) => (
                            <div key={index}>
                                <Link LinkComponent={LinkComponent} to={link} className={active ? 'font-bold' : 'text-base'}>{name}</Link>
