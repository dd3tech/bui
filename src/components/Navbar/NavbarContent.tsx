...
export type NavbarContentActiveColor = 'primary' | 'success' | 'danger' | 'warning'
...
const NavbarContent = ({ hiddenIn, children, variant = 'default', activeColor = 'primary', ...props }: NavbarContentProps) => {
    return (
        <NavbarContentProvider value={{ activeColor, variant }}>
            <div {...props} className={composeClasses('h-full flex-nowrap items-center gap-9', hiddenIn ? `hidden ${hiddenIn}:flex` : 'flex', props.className)}>
                {children}
            </div>
