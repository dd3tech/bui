...
export interface DatePickerProps {
  ...
  readonly format?: 'long' | 'short'
  readonly language?: 'es' | 'en'
  readonly value?: Date
  readonly minDate?: Date
  readonly maxDate?: Date
  readonly onChange?: (newDate: { date: string }) => void
  readonly onlyOf?: OptionType
  readonly onDaySelected?: () => void
}
...
function Calendar({
  ...
  const handleChangeSelectedDate = (newDate: Date) => {
    const [day, month, year] = newDate.toLocaleDateString('es-MX').split('/')

    const formatDay = Number(day) < 10 ? `0${day}` : day
    const formatMonth = Number(month) < 10 ? `0${month}` : month
    onChange && onChange({ date: `${formatDay}/${formatMonth}/${year}` })
