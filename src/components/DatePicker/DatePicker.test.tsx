...
describe('<DatePicker /> ', () => {
  ...
  describe('props: ', () => {
    ...
    it('onChange: prop should listen and return the date you select on the calendar.', () => {
      ...
      const { getAllByRole, getByRole } = render(
        <DatePicker onChange={onChange} onDaySelected={mockDaySelected} />
      )
  ...
  describe('buttons to go back and forward in the calendar', () => {
    describe('ui - days', () => {
      it('prevMonth button should take me to the previous month shown in the header', () => {
        ...
        expect(getByRole('select-month').firstChild?.textContent).toEqual(
          `${monthNames['en'][today.getMonth()]} ${today.getFullYear()}`
        )
      })
    ...
    describe('ui - months', () => {
      it('prevYear should take me to the previous year shown in the header', () => {
        ...
        expect(getByRole('select-year').firstChild?.textContent).toEqual(
  ...
  it('should be render with prop onlyOf=month-year', () => {
    ...
    const { getAllByRole, getByRole } = render(
      <DatePicker value={date} onlyOf="month-year" />
    )
    const monthList = getAllByRole('month')
    expect(monthList[1].innerHTML).toBe('Feb')
    fireEvent.click(getByRole('select-year'))
  ...
  it('should be render with prop minDate and maxDate', () => {
    ...
    const { getAllByRole } = render(
      <DatePicker value={date} minDate={prevDate} maxDate={nextDate} />
    )
    expect(
      getAllByRole('numberDay')[prevDate.getDate() - 2].className
    ).toContain('text-gray-300')
    expect(getAllByRole('numberDay')[nextDate.getDate()].className).toContain(
      'text-gray-300'
    )
  })
})
