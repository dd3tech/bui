...
describe('<InputCurrency/>', () => {
    ...
    it('must render the value with the indicated format', () => {
        const { getByRole } = render(<InputCurrency {...defaultProps} value={'10222222.33'} />)

        expect(getByRole('role-input').getAttribute('value')).toEqual('$10,222,222.33')
    })
    ...
    describe('Props:should be render', () => {
        ...
        it('placeholder', () => {
            ...
            expect(getByRole('role-input').getAttribute('placeholder')).toBe('Placeholder test')
        })
