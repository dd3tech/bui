...
describe('<InputFile />', () => {
    ...
    it('should render boxMessage correctly', () => {
        const { getByText } = render(<InputFile boxMessage="PDF, PNG, JPG, Peso máximo por archivo 20 MB" />)

        expect(getByText('PDF, PNG, JPG, Peso máximo por archivo 20 MB').textContent).toEqual('PDF, PNG, JPG, Peso máximo por archivo 20 MB')
    })

    it('handles onDragEnd, onDragLeave, and onDragOver', () => {
        const onChange = vi.fn()
        const { getByRole } = render(<InputFile role="input-file" onChange={onChange} />)
    ...
    it('should be change input file value', () => {
        ...
        const { getByRole } = render(<InputFile role="input-file" onChange={onChangeMock} />)
        const input = getByRole('input-file')
    ...
    it('should be show a progress indicator when this is different to 0', () => {
        const { getByRole } = render(<InputFile role="input-file" progressIndicator={40} />)
    ...
    describe('props: errorMessage and hasError', () => {
        ...
        it('when hasError is true, should be show the error message', () => {
            const { getByText } = render(<InputFile error={{ show: true, message: errorMessage }} />)
            expect(getByText(errorMessage)).toBeInTheDocument()
        })

        it('when hasError is false, should be not show the error message', () => {
            const { container } = render(<InputFile error={{ show: false, message: errorMessage }} />)
