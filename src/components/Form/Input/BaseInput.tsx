...
import { inputVariants, InputVariant as InputVariantType, InputType, getClassesByPseudoClass, getAnimationLabel, getPaddingInput } from '../shared'
...
function BaseInput({
    ...
    const isLabelScalded = !label || focused || value?.toLocaleString().length || inputRef.current?.value.length
            ...
            <div role="input-container" className={styles.container} style={style}>
                ...
                <div className="flex flex-col w-full relative h-11">
                    ...
                    <input
                        ...
                        onFocus={handleFocus}
                        onBlur={handleBlur}
                        style={{ cursor: 'inherit', zIndex: 1, ...getPaddingInput(!!label) }}
                        disabled={isDisabled}
                        value={value}
                    />
                </div>
