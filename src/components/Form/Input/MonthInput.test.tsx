...
describe('<MonthInput />', () => {
    ...
    it('DatePicker was rendering', () => {
        const { getByRole, getByTestId } = render(<MonthInput onChange={mockOnChange} data-testid="month-input" />)
    ...
    it('check default value', () => {
        const { getByTestId, rerender } = render(<MonthInput value={2} language="en" data-testid="month-input" />)
    ...
    it('we check that if we pass a default value that is outside the 12-month range, it returns an empty string', () => {
        const { getByTestId } = render(<MonthInput value={13} language="en" data-testid="month-input" />)
        const input = getByTestId('month-input') as HTMLInputElement
