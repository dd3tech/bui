    ...
    return (
        <div role={roleContainer}>
            <Text variant="p" className="mb-2 text-xs text-gray-400">
                {label}{' '}
                <label htmlFor={id} className="text-blue-700 underline underline-offset-1 cursor-pointer leading-6">
                    {labelAction}
                </label>
            </Text>
            <label
            ...
            >
                <div className={composeClasses('flex gap-3 items-center', error?.show && 'text-red-500')}>
                    <input
                        {...otherProps}
                        type="file"
                        id={id}
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        {!error?.show && !isDrag && progressIndicator !== 0 && (
                            <ProgressBar value={progressIndicator} backgroundColor="#1D4ED8" height="6px" bgColorContainer="#EFF6FF" />
                        )}
                    </div>
                </div>
            </label>
            {hintText && (
                <Text style={{ fontSize: '10px' }} variant="p" className="italic text-gray-500 font-semibold">
                    {hintText}
                </Text>
            )}
        </div>
