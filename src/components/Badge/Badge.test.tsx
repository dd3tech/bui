...
describe('<Badge/>', () => {
    ...
    it('the prop classNameIcon works correctly ', () => {
        ...
        const classNameIconElement = getByRole(badgeContainer).firstChild as HTMLDivElement
    ...
    describe('render with different variants', () => {
        it('Badge contains the className corresponding to the warning variant', () => {
            ...
            expect(getByRole(badgeContainer).className).toContain('bg-yellow-100 border border-yellow-500 text-gray-500')
        })

        it('Badge contains the className corresponding to the infoPrimary variant ', () => {
            const { getByRole } = render(<Badge variant="infoPrimary" />)
            expect(getByRole(badgeContainer).className).toContain('bg-blue-50 border border-blue-300 text-gray-500')
        })
        ...
        it('Badge contains the className corresponding to the infoSecondary variant ', () => {
            ...
            expect(getByRole(badgeContainer).className).toContain('bg-transparent border border-blue-300 text-gray-500')
        })

        it('Badge contains the className corresponding to the success variant ', () => {
            const { getByRole } = render(<Badge variant="success" />)
            expect(getByRole(badgeContainer).className).toContain('bg-green-50 border border-green-300 text-gray-500 font-medium')
        })

        it('Badge contains the className corresponding to the primary variant ', () => {
            const { getByRole } = render(<Badge variant="primary" />)
            expect(getByRole(badgeContainer).className).toContain('bg-transparent border border-blue-700 text-blue-700')
        })

        it('Badge contains the className corresponding to the secondary variant ', () => {
            const { getByRole } = render(<Badge variant="secondary" />)
            expect(getByRole(badgeContainer).className).toContain('bg-transparent border border-white text-white hover:bg-gray-50 hover:text-black')
        })
        ...
        it('Badge contains the className corresponding to the error variant and the props text works correctly  ', () => {
            ...
            expect(getByRole(badgeContainer).className).toContain('bg-red-50 border border-red-300 text-gray-500 font-medium')
