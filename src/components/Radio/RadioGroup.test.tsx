...
const RadioGroupComponent = (props: Omit<IRadioGroup, 'children' | 'onChange' | 'value'>) => {
    ...
    return (
        <RadioGroup title={props.title} name={props.name} value={selectedValue} onChange={handleChange} row={props.row}>
            <Radio label="Lorem A" value="A" inputProps={{ 'aria-label': 'A' }} className="mr-3" />
            <Radio label="Lorem B" value="B" inputProps={{ 'aria-label': 'B' }} />
            <Radio label="Lorem C" value="C" inputProps={{ 'aria-label': 'C' }} color="success" />
            NoRadioButton
        </RadioGroup>
    )
}
...
describe('<RadioGroup/>', () => {
    ...
    it('should be render each Radio with the name "radio-buttons-group"', () => {
        const { getByRole } = render(<RadioGroupComponent name="radio-buttons-group" />)
