...
describe('<FormControlLabel /> ', () => {
    it('renders a required control, which can be any jsx element', () => {
        const { getByRole } = render(<FormControlLabel control={<input role="radio" type="radio" value="Test" />} label="Test" />)
    ...
    it('Label prop is required', () => {
        const { getByTestId } = render(<FormControlLabel data-testid="control" label="Test Label" control={<p>child</p>} />)
    ...
    describe('position label', () => {
        it('top', () => {
            const { getByRole } = render(<FormControlLabel role="control" control={<Checkbox />} label="top" labelPlacement="top" />)

            expect(getByRole('control').className).toContain(directionLabel.top)
        })

        it('bottom', () => {
            const { getByRole } = render(<FormControlLabel role="control" control={<Checkbox />} label="bottom" labelPlacement="bottom" />)

            expect(getByRole('control').className).toContain(directionLabel.bottom)
        })

        it('start', () => {
            const { getByRole } = render(<FormControlLabel role="control" control={<Checkbox />} label="start" labelPlacement="start" />)

            expect(getByRole('control').className).toContain(directionLabel.start)
        })

        it('end', () => {
            const { getByRole } = render(<FormControlLabel role="control" control={<Checkbox />} label="end" labelPlacement="end" />)
