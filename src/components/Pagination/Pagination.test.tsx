...
describe('<Pagination/>', () => {
    ...
    it('should render with three dots', () => {
        const { getAllByText } = render(<Pagination {...defaultProps} currentPage={8} totalPages={15} />)
    ...
    describe('change page', () => {
        it('change to next page when next button is clicked', () => {
            const { container } = render(<Pagination {...defaultProps} />)
            const nextButton = container.querySelector('ul')?.lastChild?.firstChild as Element
        ...
        it('change to previous page when next button is clicked', () => {
            ...
            const previousButton = container.querySelector('ul')?.firstChild?.firstChild as Element
    ...
    describe('disabled buttons pagination', () => {
        it('next page is disabled when current page is on last page', () => {
            const { container } = render(<Pagination {...defaultProps} currentPage={10} totalPages={10} />)
            const nextButton = container.querySelector('ul')?.lastChild?.firstChild as Element
        ...
        it('previous page is disabled when current page is on the first page', () => {
            const { container } = render(<Pagination {...defaultProps} />)
            const firstButton = container.querySelector('ul')?.firstChild?.firstChild as Element
        ...
        it('when we click on the first page it should render with the first page', () => {
            const { getByRole } = render(<Pagination {...defaultProps} currentPage={6} />)
    ...
    describe('prop: totalPages', () => {
        ...
        it('is equal to or less than 5 it should render pages without any modifications', () => {
            const { getAllByRole } = render(<Pagination {...defaultProps} totalPages={4} />)
