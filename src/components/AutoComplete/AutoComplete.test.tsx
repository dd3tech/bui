...
describe('<AutoComplete />', () => {
    describe('render autocomplete input', () => {
        it('must allow entering alphanumeric characters', () => {
            ...
            fireEvent.change(getByRole('autocomplete'), { target: { value: 'is one test 45' } })

            expect(getByRole('autocomplete')).toHaveProperty('value', 'is one test 45')
        })
    })
    ...
    describe('disabled', () => {
        it('should have the disabled class', () => {
            ...
            expect(getByRole('autocomplete').parentElement?.className).toContain('bg-gray-100')
            expect(getByRole('autocomplete').parentElement?.className).toContain('opacity-30')
        ...
        describe('renders a different children according to the following cases', () => {
            it('renders a loading message when the property is loading', () => {
                const { getByRole, getByTestId } = render(<AutoComplete items={[]} isLoading />)
            ...
            it('renders a non-matching message when there are no elements in the array "items{[]}"', () => {
                const { getByRole } = render(<AutoComplete items={[]} isLoading={false} />)
                expect(getByRole('dialog')).toHaveTextContent('no matches')
                expect(getByRole('dialog').children[0].className).toContain('text-red-700')
            })

            it('render a list of elements when the array contains at least one element', () => {
                const { getByRole, getAllByRole } = render(<AutoComplete items={testProjectData} isLoading={false} />)
        ...
        describe('with rendered elements', () => {
            it('if I click on an element it should change the text of the input to the string of the selected element', () => {
                const { getByRole, getAllByRole } = render(<AutoComplete items={testProjectData} />)
                fireEvent.click(getAllByRole('row')[3])
                expect(getByRole('autocomplete')).toHaveProperty('value', testProjectData[3].name)
            })

            it('if i select an element it should close the container of all elements', () => {
                const { getByRole, getAllByRole } = render(<AutoComplete items={testProjectData} />)
                fireEvent.change(getByRole('autocomplete'), { target: { value: 'show items' } })
            ...
            it('should not close the element container when doing onBlur to input by default', () => {
                vi.useFakeTimers()
                const { getByRole } = render(<AutoComplete items={testProjectData} isCloseOnBlur={false} />)
                fireEvent.change(getByRole('autocomplete'), { target: { value: 'is one test 45' } })
                expect(getByRole('panel').className).not.toContain('hidden')
                fireEvent.blur(getByRole('autocomplete'))
                setTimeout(() => expect(getByRole('panel').className).not.toContain('hidden'), 300)
                vi.runAllTimers()
            })

            it('should close the element container if I do onBlur and the property isCloseOnBlur="true"', () => {
                vi.useFakeTimers()
                const { getByRole } = render(<AutoComplete items={testProjectData} isCloseOnBlur={true} />)
                fireEvent.change(getByRole('autocomplete'), { target: { value: 'is one test 45' } })
                expect(getByRole('panel').className).not.toContain('hidden')
                fireEvent.blur(getByRole('autocomplete'))
                setTimeout(() => expect(getByRole('panel').className).toContain('hidden'), 300)
                vi.runAllTimers()
            })

            it('the element container should appear another round on input focus only if the property isCloseOnBlur={true} after onBlur', () => {
                vi.useFakeTimers()
                const { getByRole } = render(<AutoComplete items={testProjectData} value="is ok" isCloseOnBlur={true} />)
                fireEvent.change(getByRole('autocomplete'), { target: { value: 'show container' } })
                expect(getByRole('panel').className).not.toContain('hidden')
                fireEvent.blur(getByRole('autocomplete'))
                setTimeout(() => expect(getByRole('panel').className).toContain('hidden'), 300)
                vi.runAllTimers()
        ...
        it('executing the onChange function', () => {
            ...
            const { getByRole } = render(<AutoComplete onChange={onChange} value={value} items={testProjectData} />)
            fireEvent.change(getByRole('autocomplete'), { target: { value: 'hello test' } })
        ...
        describe('should respects the componentsProps if passed', () => {
            it('canFindText is working', () => {
                ...
                const { getByRole } = render(<AutoComplete items={[]} canFindText={canFindText} />)
            ...
            it('loadingText is working', () => {
                ...
                const { getByRole } = render(<AutoComplete items={[]} isLoading loadingText={loadingText} />)
            ...
            it('placeholder is working', () => {
                ...
                const { getByRole } = render(<AutoComplete items={[]} placeholder={placeholder} />)

                expect(getByRole('autocomplete')).toHaveProperty('placeholder', placeholder)
            })
        })
    })
})
