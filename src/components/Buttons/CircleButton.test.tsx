...
describe('<BaseCircleButton />', () => {
    ...
    it('should be render CircleButton with an Icon', () => {
        const { container } = render(<Button.CircleButton {...defaultProps} Icon={() => <MailIcon id="MailIcon" />} />)
        const icon = container.querySelector('#cell-icon')?.querySelector('#MailIcon')
        expect(icon).not.toBeNull()
    })

    it('should be render the BaseCircleButton with the specified width and height', () => {
        const { container } = render(<Button.BaseCircleButton {...defaultProps} width="3rem" />)
    ...
    it('should be render the BaseCircleButton with the shadow sm', () => {
        const { container } = render(<Button.BaseCircleButton {...defaultProps} shadow="sm" />)
        expect(container.firstChild).toHaveClass('shadow-sm')
    })

    it('should be render the BaseCircleButton disabled', () => {
        const { container } = render(<Button.BaseCircleButton {...defaultProps} disabled={true} />)
