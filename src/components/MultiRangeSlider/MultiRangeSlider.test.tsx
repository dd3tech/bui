...
describe('<MultiRangeSlider/>', () => {
    ...
    describe('prop: minValDisabled, maxValDisabled', () => {
        it('should render a MultiRangeSlider with both controllers disabled', () => {
            const { container } = render(<MultiRangeSlider {...defaultProps} minValDisabled maxValDisabled />)
            const minInput = container.querySelector('input[name="min-val"]') as HTMLInputElement
            const maxInput = container.querySelector('input[name="max-val"]') as HTMLInputElement
    ...
    it('updateBar should update the style of the range div', () => {
        const range: RefObject<HTMLDivElement> = { current: document.createElement('div') }
