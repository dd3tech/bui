...
describe('composeClasses function', () => {
    ...
    it('should concatenate the parameters that we pass to it', () => {
        expect(composeClasses(mockWords.first, mockWords.second)).toBe(`${mockWords.first} ${mockWords.second}`)
    })

    it('if we pass any argument in undefined it should remove it', () => {
        expect(composeClasses(undefined)).toBe('')
        expect(composeClasses(mockWords.first, undefined, mockWords.three)).toBe(`${mockWords.first} ${mockWords.three}`)
    ...
    it('if we pass any argument in null it should remove it', () => {
        ...
        expect(composeClasses(mockWords.first, null, mockWords.three)).toBe(`${mockWords.first} ${mockWords.three}`)
        expect(composeClasses(mockWords.first, null)).toBe(mockWords.first)
    })

    it('should remove booleans', () => {
        expect(composeClasses(false)).toBe('')
        expect(composeClasses(mockWords.first, false, mockWords.three)).toBe(`${mockWords.first} ${mockWords.three}`)
