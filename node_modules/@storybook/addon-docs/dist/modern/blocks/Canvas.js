...
const getPreviewProps = (_ref, docsContext, sourceContext) => {
  ...
  const stories = childArray.filter(c => c.props && (c.props.id || c.props.name));
  const targetIds = stories.map(s => s.props.id || toId(mdxComponentMeta.id || mdxComponentMeta.title, storyNameFromExport(mdxStoryNameToKey[s.props.name])));
